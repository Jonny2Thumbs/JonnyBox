...
function resizeCanvasToImage(){
  let maxWidth=window.innerWidth*0.66;
  let maxHeight=window.innerHeight*0.66;
  let w=image.width;
  let h=image.height;
...
// Touch support
function getTouchPos(e){ const rect=canvas.getBoundingClientRect(); const t=e.touches[0]||e.changedTouches[0]; return{x:t.clientX-rect.left, y:t.clientY-rect.top}; }
canvas.addEventListener('touchstart', e=>{ if(!image.src) return; e.preventDefault(); drawing=true; currentPath=[]; const {x,y}=getTouchPos(e); if(currentMode==='screw'||currentMode==='bolt'){ addMarker(currentMode,[{x,y}]); drawing=false; } else currentPath.push({x,y}); }, {passive:false});
canvas.addEventListener('touchmove', e=>{ if(!drawing) return; e.preventDefault(); const {x,y}=getTouchPos(e); currentPath.push({x,y}); redrawMarkers(); if(currentMode==='component'||currentMode==='custom'){ ctx.strokeStyle=colorForType(currentMode); ctx.lineWidth=3; ctx.beginPath(); currentPath.forEach((p,i)=> i===0?ctx.moveTo(p.x,p.y):ctx.lineTo(p.x,p.y)); ctx.stroke(); } }, {passive:false});
canvas.addEventListener('touchend', e=>{ if(!drawing) return; drawing=false; if(currentMode==='component'||currentMode==='custom'){ if(currentPath.length>0) addMarker(currentMode,currentPath); currentPath=[]; } }, {passive:false});
